...
func (gq *GroupQuery) QueryStudents() *UserQuery {
...
// QueryGroupRun chains the current query on the "group_run" edge.
func (gq *GroupQuery) QueryGroupRun() *GroupRunQuery {
	query := (&GroupRunClient{config: gq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withStudents: gq.withStudents.Clone(),
		withCourse:   gq.withCourse.Clone(),
		withGroupRun: gq.withGroupRun.Clone(),
...
func (gq *GroupQuery) WithStudents(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withStudents = query
	return gq
}
...
// WithGroupRun tells the query-builder to eager-load the nodes that are connected to
// the "group_run" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithGroupRun(opts ...func(*GroupRunQuery)) *GroupQuery {
	query := (&GroupRunClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupRun = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if gq.withCourse != nil || gq.withGroupRun != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := gq.withStudents; query != nil {
		if err := gq.loadStudents(ctx, query, nodes,
			func(n *Group) { n.Edges.Students = []*User{} },
			func(n *Group, e *User) { n.Edges.Students = append(n.Edges.Students, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := gq.withGroupRun; query != nil {
		if err := gq.loadGroupRun(ctx, query, nodes, nil,
			func(n *Group, e *GroupRun) { n.Edges.GroupRun = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadStudents(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.StudentsTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.StudentsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(group.StudentsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.StudentsPrimaryKey[0]))
...
func (gq *GroupQuery) loadCourse(ctx context.Context, query *CourseQuery, nodes []*Group, init func(*Group), assign func(*Group, *Course)) error {
	...
	for i := range nodes {
		if nodes[i].course_groups == nil {
			continue
		}
		fk := *nodes[i].course_groups
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "course_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (gq *GroupQuery) loadGroupRun(ctx context.Context, query *GroupRunQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupRun)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Group)
	for i := range nodes {
		if nodes[i].group_run_groups == nil {
			continue
		}
		fk := *nodes[i].group_run_groups
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_run_groups" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
