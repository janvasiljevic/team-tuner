...
func (tx *Tx) init() {
	tx.BfiAnswer = NewBfiAnswerClient(tx.config)
	tx.BfiQuestion = NewBfiQuestionClient(tx.config)
	tx.BfiReport = NewBfiReportClient(tx.config)
	tx.Course = NewCourseClient(tx.config)
	tx.Group = NewGroupClient(tx.config)
	tx.GroupRun = NewGroupRunClient(tx.config)
...
// applies a query, for example: BfiAnswer.QueryXXX(), the query will be executed
